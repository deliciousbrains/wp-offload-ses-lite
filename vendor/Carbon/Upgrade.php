...
use DeliciousBrains\WP_Offload_SES\Composer\Composer;
use DeliciousBrains\WP_Offload_SES\Composer\Config;
use DeliciousBrains\WP_Offload_SES\Composer\IO\ConsoleIO;
use DeliciousBrains\WP_Offload_SES\Composer\Script\Event as ScriptEvent;
use DeliciousBrains\WP_Offload_SES\Symfony\Component\Console\Helper\HelperSet;
use DeliciousBrains\WP_Offload_SES\Symfony\Component\Console\Helper\QuestionHelper;
use DeliciousBrains\WP_Offload_SES\Symfony\Component\Console\Input\StringInput;
use DeliciousBrains\WP_Offload_SES\Symfony\Component\Console\Output\ConsoleOutput;
use DeliciousBrains\WP_Offload_SES\UpdateHelper\UpdateHelper;
use DeliciousBrains\WP_Offload_SES\UpdateHelper\UpdateHelperInterface;
class Upgrade implements UpdateHelperInterface
{
    ...
    public static function upgrade(ScriptEvent $event = null)
    {
        if (!$event) {
            ...
            $composer->setConfig(new Config(\true, $baseDir));
            $event = new ScriptEvent('upgrade-carbon', $composer, new ConsoleIO(new StringInput(''), new ConsoleOutput(), new HelperSet(array(new QuestionHelper()))));
        }
        static::askForUpgrade(new UpdateHelper($event), \true);
    }
}
