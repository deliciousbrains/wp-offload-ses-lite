...
final class DatePoint extends \DateTimeImmutable
{
        ...
        if ('now' !== $datetime) {
            ...
            if (\PHP_VERSION_ID < 80300) {
                try {
                    ...
                    throw new \DeliciousBrains\WP_Offload_SES\DateMalformedStringException($e->getMessage(), $e->getCode(), $e);
                }
            } else {
                $builtInDate = new parent($datetime, $timezone ?? $now->getTimezone());
                $timezone = $builtInDate->getTimezone();
    ...
    {
        return parent::createFromFormat($format, $datetime, $timezone) ?: throw new \DeliciousBrains\WP_Offload_SES\DateMalformedStringException(static::getLastErrors()['errors'][0] ?? 'Invalid date string or format.');
    }
    ...
    {
        if (\PHP_VERSION_ID < 80300) {
            return @parent::modify($modifier) ?: throw new \DeliciousBrains\WP_Offload_SES\DateMalformedStringException(\error_get_last()['message'] ?? \sprintf('Invalid modifier: "%s".', $modifier));
        }
    ...
    {
        return parent::getTimezone() ?: throw new \DeliciousBrains\WP_Offload_SES\DateInvalidTimeZoneException('The DatePoint object has no timezone.');
    }
}
