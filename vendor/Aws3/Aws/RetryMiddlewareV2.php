...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\ConfigurationInterface;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\QuotaManager;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\RateLimiter;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\RetryHelperTrait;
...
class RetryMiddlewareV2
{
    ...
    public static function createDefaultDecider(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\QuotaManager $quotaManager, $maxAttempts = 3, $options = [])
    {
        $retryCurlErrors = [];
        if (extension_loaded('curl')) {
            $retryCurlErrors[CURLE_RECV_ERROR] = true;
        }
        return function ($attempts, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $command, $result) use($options, $quotaManager, $retryCurlErrors, $maxAttempts) {
    ...
    public function __construct(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\ConfigurationInterface $config, callable $handler, $options = [])
    {
        $this->options = $options;
        $this->maxAttempts = $config->getMaxAttempts();
        $this->mode = $config->getMode();
        $this->nextHandler = $handler;
        $this->quotaManager = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\QuotaManager();
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = isset($options['rate_limiter']) ? $options['rate_limiter'] : new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\RateLimiter();
        }
    }
    public function __invoke(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $req)
    {
