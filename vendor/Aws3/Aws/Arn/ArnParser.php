...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\S3\AccessPointArn as S3AccessPointArn;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\S3\OutpostsBucketArn;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\S3\RegionalBucketArn;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\S3\OutpostsAccessPointArn;
/**
...
class ArnParser
{
    ...
    {
        $data = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\Arn::parse($string);
        $resource = self::explodeResourceComponent($data['resource']);
        if ($resource[0] === 'outpost') {
            if (isset($resource[2]) && $resource[2] === 'bucket') {
                return new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\S3\OutpostsBucketArn($string);
            }
            if (isset($resource[2]) && $resource[2] === 'accesspoint') {
                return new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\S3\OutpostsAccessPointArn($string);
            }
        }
        if ($resource[0] === 'accesspoint') {
            if ($data['service'] === 's3') {
                return new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\S3\AccessPointArn($string);
            }
            return new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\AccessPointArn($string);
        }
        return new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Arn\Arn($data);
    }
