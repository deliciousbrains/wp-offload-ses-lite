...
namespace DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery;

use DeliciousBrains\WP_Offload_SES\Aws3\Aws\AwsClient;
...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Middleware;
...
class EndpointDiscoveryMiddleware
{
    ...
    public function __construct(callable $handler, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\AwsClient $client, array $args, $config)
    {
    ...
    public function __invoke(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
        $op = $this->service->getOperation($cmd->getName())->toArray();
        // Continue only if endpointdiscovery trait is set
        if (isset($op['endpointdiscovery'])) {
            $config = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery\ConfigurationProvider::unwrap($this->config);
            $isRequired = !empty($op['endpointdiscovery']['required']);
            if ($isRequired && !$config->isEnabled()) {
                throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\UnresolvedEndpointException('This operation ' . 'requires the use of endpoint discovery, but this has ' . 'been disabled in the configuration. Enable endpoint ' . 'discovery or use a different operation.');
            }
            // Continue only if enabled by config
            if ($config->isEnabled()) {
                if (isset($op['endpointoperation'])) {
                    throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\UnresolvedEndpointException('This operation is ' . 'contradictorily marked both as using endpoint discovery ' . 'and being the endpoint discovery operation. Please ' . 'verify the accuracy of your model files.');
                }
                ...
                // Check/create cache
                if (!isset(self::$cache)) {
                    self::$cache = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\LruArrayCache($config->getCacheLimit());
                }
                if (empty($endpointList = self::$cache->get($cacheKey))) {
                    $endpointList = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery\EndpointList([]);
                }
    ...
    private function discoverEndpoint($cacheKey, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (isset($result['Endpoints'])) {
            ...
            $endpointList = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery\EndpointList($endpointData);
            self::$cache->set($cacheKey, $endpointList);
            return $endpointList->getEndpoint();
        }
        throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\UnresolvedEndpointException('The endpoint discovery operation ' . 'yielded a response that did not contain properly formatted ' . 'endpoint data.');
    }
    private function getCacheKey(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\CredentialsInterface $creds, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
    ...
    private function getDiscoveryCommand(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (!isset($endpointOperation)) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\UnresolvedEndpointException('This command is set to use ' . 'endpoint discovery, but no endpoint discovery operation was ' . 'found. Please verify the accuracy of your model files.');
        }
        $params = [];
        if (!empty($identifiers)) {
        ...
        $command->getHandlerList()->appendBuild(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Middleware::mapRequest(function (\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $r) {
    ...
    private function handleDiscoveryException($isRequired, $originalUri, \Exception $e, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($isRequired) {
            ...
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\AwsException($message, $cmd, ['code' => 'EndpointDiscoveryException', 'message' => $message], $e);
        }
    ...
    private function modifyRequest(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request, $endpoint)
    {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\UnresolvedEndpointException("The supplied endpoint '" . "{$endpoint}' is invalid.");
    }
    private function useOriginalUri(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\UriInterface $uri, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\CommandInterface $cmd, \DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
