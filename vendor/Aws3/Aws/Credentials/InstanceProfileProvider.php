...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\InvalidJsonException;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Sdk;
use DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        if (strcasecmp($disabled, 'true') === 0) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException($this->createErrorMessage('EC2 metadata service access disabled'));
        }
        $fn = $this->client;
        $request = new \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Psr7\Request($method, self::SERVER_URI . $url);
        $userAgent = 'aws-sdk-php/' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Sdk::VERSION;
        if (defined('HHVM_VERSION')) {
            $userAgent .= ' HHVM/' . HHVM_VERSION;
        }
        $userAgent .= ' ' . \DeliciousBrains\WP_Offload_SES\Aws3\Aws\default_user_agent();
    ...
    private function handleRetryableException(\Exception $e, $retryOptions, $message)
    {
        ...
        if ($isRetryable && $this->attempts < $this->retries) {
            ...
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException($message);
        }
    }
    ...
    private function decodeResult($response)
    {
        ...
        if (json_last_error() > 0) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\InvalidJsonException();
        }
