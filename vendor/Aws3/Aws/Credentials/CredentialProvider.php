...
class CredentialProvider
{
    ...
    public static function sso($ssoProfileName, $filename = null, $config = [])
    {
        ...
        return function () use($ssoProfileName, $filename, $config) {
            ...
            try {
                $expiration = (new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\DateTimeResult($tokenData['expiresAt']))->getTimestamp();
            } catch (\Exception $e) {
            ...
            if (empty($config['ssoClient'])) {
                $ssoClient = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\SSO\SSOClient(['region' => $ssoProfile['sso_region'], 'version' => '2019-06-10', 'credentials' => false]);
            } else {
            ...
            return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\promise_for(new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\Credentials($ssoCredentials['accessKeyId'], $ssoCredentials['secretAccessKey'], $ssoCredentials['sessionToken'], $expiration));
        };
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\promise_for(new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient, $config = [])
    {
        ...
        if (empty($stsClient)) {
            ...
            if (!empty($roleProfile['source_profile'])) {
                $sourceCredentials = call_user_func(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            } else {
                $sourceCredentials = self::getCredentialsFromSource($profileName, $filename);
            }
            $stsClient = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $credentials = $stsClient->createCredentials($result);
        return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\promise_for($credentials);
    }
    /**
    ...
    {
        $profileData = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (file_exists($credFile)) {
            $profiles = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }
        if (file_exists($configFile)) {
            $configProfileData = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    public static function getCredentialsFromSource($profileName = '', $filename = '', $config = [])
    {
        ...
        switch ($credentialSource) {
            ...
            case 'EcsContainer':
                ...
                break;
            default:
                throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\CredentialsException("Invalid credential_source found in config file: {$credentialSource}. Valid inputs " . "include Environment, Ec2InstanceMetadata, and EcsContainer.");
        }
        ...
        return function () use($credentialsResult) {
            return \DeliciousBrains\WP_Offload_SES\Aws3\GuzzleHttp\Promise\promise_for($credentialsResult);
        };
    }
