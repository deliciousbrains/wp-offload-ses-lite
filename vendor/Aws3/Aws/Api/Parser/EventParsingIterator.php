...
use Iterator;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\EventStreamDataException;
...
class EventParsingIterator implements \Iterator
{
    ...
    /** @var AbstractParser */
    private $parser;
    public function __construct(\DeliciousBrains\WP_Offload_SES\Aws3\Psr\Http\Message\StreamInterface $stream, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\StructureShape $shape, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Parser\AbstractParser $parser)
    {
        $this->decodingIterator = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Parser\DecodingEventStreamIterator($stream);
        $this->shape = $shape;
        $this->parser = $parser;
    }
    ...
    private function parseEvent(array $event)
    {
        if (!empty($event['headers'][':message-type'])) {
            ...
            if ($event['headers'][':message-type'] !== 'event') {
                throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Parser\Exception\ParserException('Failed to parse unknown message type.');
            }
        }
        if (empty($event['headers'][':event-type'])) {
            throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Parser\Exception\ParserException('Failed to parse without event type.');
        }
    ...
    {
        throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\EventStreamDataException($event['headers'][':error-code'], $event['headers'][':error-message']);
    }
}
