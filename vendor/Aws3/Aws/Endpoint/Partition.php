...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\S3\RegionalEndpoint\ConfigurationProvider as S3ConfigurationProvider;
...
final class Partition implements \ArrayAccess, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Endpoint\PartitionInterface
{
    ...
    {
        return $service === 'sts' && in_array($region, $this->stsLegacyGlobalRegions) && (empty($options['sts_regional_endpoints']) || \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Sts\RegionalEndpoints\ConfigurationProvider::unwrap($options['sts_regional_endpoints'])->getEndpointsType() !== 'regional');
    }
    /**
    ...
    {
        return $service === 's3' && $region === 'us-east-1' && (empty($options['s3_us_east_1_regional_endpoint']) || \DeliciousBrains\WP_Offload_SES\Aws3\Aws\S3\RegionalEndpoint\ConfigurationProvider::unwrap($options['s3_us_east_1_regional_endpoint'])->getEndpointsType() !== 'regional');
    }
