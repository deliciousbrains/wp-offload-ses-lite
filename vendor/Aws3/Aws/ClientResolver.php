...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\ClientSideMonitoring\Configuration;
...
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery\ConfigurationInterface;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery\ConfigurationProvider;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\InvalidRegionException;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
...
class ClientResolver
{
    ...
    public static function _apply_retries($value, array &$args, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\HandlerList $list)
    {
        ...
        if ($value) {
            $config = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    ...
    {
        return \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\HandlerList $list)
    {
        if ($value === false) {
            $value = new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\ClientSideMonitoring\Configuration(false, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \DeliciousBrains\WP_Offload_SES\Aws3\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\DeliciousBrains\WP_Offload_SES\Aws3\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        ...
        $args['error_parser'] = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Exception\InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \DeliciousBrains\WP_Offload_SES\Aws3\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    ...
    {
        if ($value !== false) {
            $list->interpose(new \DeliciousBrains\WP_Offload_SES\Aws3\Aws\TraceMiddleware($value === true ? [] : $value, $args['api']));
        }
    }
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        return \DeliciousBrains\WP_Offload_SES\Aws3\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
